...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should create status when logged in" do
    ...
    assert_difference('Status.count') do
      post :create, status: { content: @status.content, user_id: users(:sam) }
    end
  ...
  test "should create status for the current user when logged in" do
    ...
    assert_difference('Status.count') do
      post :create, status: { content: @status.content, user_id: users(:jan) }
    end

    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:sam).id
  end
  ...
  test "should update status for current user when logged in" do
    sign_in users(:sam)
    put :update, id: @status, status: { content: @status.content, user_id: users(:jan).id }
    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:sam).id
  end
